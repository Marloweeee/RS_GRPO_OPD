...
STEP_RE = re.compile(
    r"global_step/max_steps': '([0-9]+/[0-9]+)'|global_step/max_steps=([0-9]+/[0-9]+)"
)
CKPT_RE = re.compile(
    r"(?:latest_ckpt=|last_model_checkpoint: |Saving model checkpoint to )"
...
def summarize_once() -> str:
    ...
    for name, log_path in RUNS:
        ...
        lines.append(
            f"{name}: step={step} latest_step={ckpt_step} latest_ckpt={ckpt_path} "

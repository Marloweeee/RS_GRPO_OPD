#!/usr/bin/env bash
# Continue from the clean RRSIS-D SFT checkpoint with the strongest current
# greedy-test GRPO+OPSD setting: metric-triggered teacher refresh + gaussian hint.
# Training runs on two gui_agent 8-GPU replicas, then a detached GPU eval job tests
# the latest checkpoint on rs_test.jsonl.
set -euo pipefail
...
experiment_slug="${EXPERIMENT_SLUG:-sft-to-opsd-metric-gaussian}"
run_name="${RUN_NAME:-gui-sd-qwen3-4b-rrsisd_sft_grpo_opsd_metric_gaussian_e1-${timestamp}}"
rjob_name="${RJOB_NAME:-q4b-sftopsd-gauss-e1-${timestamp}}"
...
eval_rjob_name="${EVAL_RJOB_NAME:-q4b-sftopsd-test-${timestamp}}"
...
# Make the local log follower terminate after both replicas print their worker
# completion line, so the tmux pane does not keep a finished rjob alive.
...
# Student, teacher, ref, and rollout server all start from the clean SFT checkpoint.
# The worker uses MODEL_PATH as --ref_model, so setting MODEL_PATH is enough for ref.
...
# This is a continuation from a strong SFT policy, so use the lower LR from the
# current best extra-epoch gaussian run rather than the base-from-scratch LR.
...
echo "[sft-to-opsd] timestamp=${timestamp}"
echo "[sft-to-opsd] source_ckpt=${source_ckpt}"
echo "[sft-to-opsd] run_name=${run_name}"
echo "[sft-to-opsd] train_log=${train_log}"
echo "[sft-to-opsd] eval_root=${eval_root}"
...
echo "[sft-to-opsd] latest_ckpt=${latest_ckpt}"
echo "[sft-to-opsd] launching detached test eval"
MODEL="$latest_ckpt" \
...
echo "[sft-to-opsd] waiting for eval summary=${summary_path}"
for _ in $(seq 1 "${EVAL_WAIT_POLLS:-240}"); do
    if [ -f "$summary_path" ]; then
        echo "[sft-to-opsd] eval summary ready"
        cat "$summary_path"
        echo
        echo "[sft-to-opsd] completed"
        exit 0
    fi

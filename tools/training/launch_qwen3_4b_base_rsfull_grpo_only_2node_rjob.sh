#!/usr/bin/env bash
# Launch the two-node Qwen3-VL-4B full rs_full GRPO-only run.
#
# This script runs a predict-only resource probe first, then launches two rjob
# replicas. Each replica runs run_qwen3_4b_base_rsfull_grpo_only_2node_worker.sh.
set -euo pipefail
...
run_name="${RUN_NAME:-gui-sd-qwen3-4b-base_rsfull_grpo_only_2node_bsz4_gacc2_lr2e6_e1-${timestamp}}"
rjob_name="${RJOB_NAME:-q4b-grpoonly-2n-${timestamp}}"
log_path="${RJOB_LOG_PATH:-/data/codes/gui_grounding/data/logs/${rjob_name}.rjob.log}"
worker_script="${WORKER_SCRIPT:-${repo_root}/tools/training/run_qwen3_4b_base_rsfull_grpo_only_2node_worker.sh}"
...
echo "[launch-qwen3-4b-grpo-only-2node] predict: ${predict_cmd[*]}"
"${predict_cmd[@]}"
...
echo "[launch-qwen3-4b-grpo-only-2node] run_name=${run_name}"
echo "[launch-qwen3-4b-grpo-only-2node] rjob_name=${rjob_name}"
echo "[launch-qwen3-4b-grpo-only-2node] log_path=${log_path}"
echo "[launch-qwen3-4b-grpo-only-2node] group=${group}"
echo "[launch-qwen3-4b-grpo-only-2node] charged_group=${charged_group}"
echo "[launch-qwen3-4b-grpo-only-2node] worker_script=${worker_script}"
echo "[launch-qwen3-4b-grpo-only-2node] base_model_path=${base_model_path}"
echo "[launch-qwen3-4b-grpo-only-2node] train_jsonl=${train_jsonl}"
echo "[launch-qwen3-4b-grpo-only-2node] test_jsonl=${test_jsonl}"
echo "[launch-qwen3-4b-grpo-only-2node] ckpt_root=${ckpt_root}"
echo "[launch-qwen3-4b-grpo-only-2node] artifact_root=${artifact_root}"
echo "[launch-qwen3-4b-grpo-only-2node] per_device_train_batch_size=${PER_DEVICE_TRAIN_BATCH_SIZE:-4}"
echo "[launch-qwen3-4b-grpo-only-2node] gradient_accumulation_steps=${GRADIENT_ACCUMULATION_STEPS:-2}"
echo "[launch-qwen3-4b-grpo-only-2node] lr=${LR:-2e-6}"
echo "[launch-qwen3-4b-grpo-only-2node] command: ${launch_cmd[*]}"

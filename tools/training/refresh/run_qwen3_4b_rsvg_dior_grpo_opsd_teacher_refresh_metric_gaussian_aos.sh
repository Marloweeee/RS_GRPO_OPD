#!/usr/bin/env bash
# Qwen3-VL-4B RSVG-DIOR GRPO+OPSD one-epoch run.
# Uses the metric-triggered gaussian teacher-refresh setting that has been the
# strongest stable 4B configuration in the previous RS experiments.
set -euo pipefail
...
export EXPERIMENT_SLUG="${EXPERIMENT_SLUG:-rsvg-dior-teacher-refresh-metric-gaussian}"
export RUN_NAME="${RUN_NAME:-gui-sd-qwen3-4b-rsvg_dior_grpo_opsd_teacher_refresh_metric_gaussian_e1-${timestamp}}"
export RJOB_NAME="${RJOB_NAME:-q4b-rsvg-trmetric-gauss-2n-${timestamp}}"
...
export CKPT_ROOT="${CKPT_ROOT:-/mnt/jfs/copilot/lhb/checkpoint/rs/rsvg-dior}"
export ARTIFACT_ROOT="${ARTIFACT_ROOT:-/mnt/jfs/copilot/lhb/artifacts/rs/rsvg-dior}"
export TRAIN_JSONL="${TRAIN_JSONL:-/data/codes/gui_grounding/data/rsvg_dior_full/rsvg_train.jsonl}"
export TEST_JSONL="${TEST_JSONL:-/data/codes/gui_grounding/data/rsvg_dior_full/rsvg_test.jsonl}"

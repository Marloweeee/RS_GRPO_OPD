#!/usr/bin/env bash
# Qwen3-VL-4B rs_full GRPO+SDPO with slow teacher EMA plus one-shot EWMA refresh.
# Teacher behavior:
#   - slow EMA every optimizer step: teacher = 0.98 * teacher + 0.02 * student
#   - one hard refresh when EWMA/dual-window quality and stability checks pass
#   - ref model stays fixed at the base checkpoint
# Mask mode: zoom_in.
set -euo pipefail
...
export EXPERIMENT_SLUG="${EXPERIMENT_SLUG:-teacher-ema098-refresh-ewma-zoom-in}"
export RUN_NAME="${RUN_NAME:-gui-sd-qwen3-4b-rsfull_grpo_sdpo_teacher_ema098_refresh_ewma_zoom_in_e1-${timestamp}}"
export RJOB_NAME="${RJOB_NAME:-q4b-tema098-ewma-zoom-2n-${timestamp}}"
...
export VLLM_SERVER_PORT="${VLLM_SERVER_PORT:-8794}"
export MASTER_PORT="${MASTER_PORT:-30020}"
...
export SDPO_TEACHER_REFRESH_WARMUP="${SDPO_TEACHER_REFRESH_WARMUP:-0}"

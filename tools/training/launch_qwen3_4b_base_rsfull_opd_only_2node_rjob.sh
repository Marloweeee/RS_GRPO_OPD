#!/usr/bin/env bash
# Launch the two-node Qwen3-VL-4B full rs_full OPD/OPSD-only run.
#
# This script runs a predict-only resource probe first, then launches two rjob
# replicas. Each replica runs run_qwen3_4b_base_rsfull_opd_only_2node_worker.sh.
set -euo pipefail
...
run_name="${RUN_NAME:-gui-sd-qwen3-4b-base_rsfull_opd_only_2node_bsz4_gacc2_lr2e6_e1-${timestamp}}"
rjob_name="${RJOB_NAME:-q4b-opdonly-2n-${timestamp}}"
log_path="${RJOB_LOG_PATH:-/data/codes/gui_grounding/data/logs/${rjob_name}.rjob.log}"
worker_script="${WORKER_SCRIPT:-${repo_root}/tools/training/run_qwen3_4b_base_rsfull_opd_only_2node_worker.sh}"
...
echo "[launch-qwen3-4b-opd-only-2node] predict: ${predict_cmd[*]}"
"${predict_cmd[@]}"
...
launch_cmd=(
    ...
    --set-env "SAVE_STEPS=${SAVE_STEPS:-50}"
    --set-env "SAVE_TOTAL_LIMIT=${SAVE_TOTAL_LIMIT:-10}"
    ...
    --set-env "OPSD_MASK_MODE=${OPSD_MASK_MODE:-gaussian}"
    --set-env "OPSD_HINT_MODE=${OPSD_HINT_MODE:-hint}"
    --set-env "OPSD_HINT_BOX_COLOR=${OPSD_HINT_BOX_COLOR:-magenta}"
    --set-env "OPSD_JITTER_RATIO=${OPSD_JITTER_RATIO:-0.2}"
    --set-env "OPSD_EMA_DECAY=${OPSD_EMA_DECAY:-0.95}"
    --set-env "OPSD_MONITOR_TEACHER=${OPSD_MONITOR_TEACHER:-false}"
    --set-env "LMBDA=${LMBDA:-1}"
    --set-env "SEQ_KD=${SEQ_KD:-false}"
    --set-env "OPD_BETA=${OPD_BETA:-1}"
...
echo "[launch-qwen3-4b-opd-only-2node] run_name=${run_name}"
echo "[launch-qwen3-4b-opd-only-2node] rjob_name=${rjob_name}"
echo "[launch-qwen3-4b-opd-only-2node] log_path=${log_path}"
echo "[launch-qwen3-4b-opd-only-2node] group=${group}"
echo "[launch-qwen3-4b-opd-only-2node] charged_group=${charged_group}"
echo "[launch-qwen3-4b-opd-only-2node] worker_script=${worker_script}"
echo "[launch-qwen3-4b-opd-only-2node] base_model_path=${base_model_path}"
echo "[launch-qwen3-4b-opd-only-2node] train_jsonl=${train_jsonl}"
echo "[launch-qwen3-4b-opd-only-2node] test_jsonl=${test_jsonl}"
echo "[launch-qwen3-4b-opd-only-2node] ckpt_root=${ckpt_root}"
echo "[launch-qwen3-4b-opd-only-2node] artifact_root=${artifact_root}"
echo "[launch-qwen3-4b-opd-only-2node] per_device_train_batch_size=${PER_DEVICE_TRAIN_BATCH_SIZE:-4}"
echo "[launch-qwen3-4b-opd-only-2node] gradient_accumulation_steps=${GRADIENT_ACCUMULATION_STEPS:-2}"
echo "[launch-qwen3-4b-opd-only-2node] lr=${LR:-2e-6}"
echo "[launch-qwen3-4b-opd-only-2node] lmbda=${LMBDA:-1} opd_beta=${OPD_BETA:-1} opsd_ema_decay=${OPSD_EMA_DECAY:-0.95}"
echo "[launch-qwen3-4b-opd-only-2node] command: ${launch_cmd[*]}"

#!/usr/bin/env bash
# Sibling-guided MRPD experiment.
#
# Student/teacher/ref/rollout all start from the clean RRSIS-D SFT checkpoint.
# Failed rollouts are corrected by the teacher, but the teacher visual hint is
# built from the best successful sibling trajectory in the same rollout group
# when available; groups without a good sibling fall back to GT hints.
set -euo pipefail
...
experiment_slug="${EXPERIMENT_SLUG:-sft-mrpd-sibling-gtfallback-softwindow}"
run_name="${RUN_NAME:-gui-sd-qwen3-4b-rrsisd_sft_mrpd_sibling_gtfallback_softwindow_e1-${timestamp}}"
rjob_name="${RJOB_NAME:-q4b-sftmrpd-sibling-sw-e1-${timestamp}}"
...
eval_rjob_name="${EVAL_RJOB_NAME:-q4b-sibling-sw-test-${timestamp}}"
...
export VLLM_SERVER_PORT="${VLLM_SERVER_PORT:-8893}"
...
export SDPO_HINT_SOURCE="${SDPO_HINT_SOURCE:-sibling_gt_fallback}"
...
echo "[sibling-softwindow] timestamp=${timestamp}"
echo "[sibling-softwindow] source_ckpt=${source_ckpt}"
echo "[sibling-softwindow] run_name=${run_name}"
echo "[sibling-softwindow] train_log=${train_log}"
echo "[sibling-softwindow] eval_root=${eval_root}"
...
echo "[sibling-softwindow] latest_ckpt=${latest_ckpt}"
echo "[sibling-softwindow] launching detached test eval"
MODEL="$latest_ckpt" \
...
echo "[sibling-softwindow] waiting for eval summary=${summary_path}"
for _ in $(seq 1 "${EVAL_WAIT_POLLS:-240}"); do
    if [ -f "$summary_path" ]; then
        echo "[sibling-softwindow] eval summary ready"
        cat "$summary_path"
        echo
        echo "[sibling-softwindow] completed"
        exit 0
    fi

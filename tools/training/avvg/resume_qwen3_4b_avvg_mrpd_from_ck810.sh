#!/usr/bin/env bash
# Resume AVVG-refGeo MRPD training from checkpoint-810.
# Student and teacher both start from ckpt-810; ref_model uses base 4B-Instruct.
set -euo pipefail
...
source_ckpt="${SOURCE_CKPT:-/mnt/jfs/copilot/lhb/checkpoint/rs/avvg-mrpd/gui-sd-qwen3-4b-avvg_refgeo_sft_mrpd_metric_gaussian_e1_resume750_aos-20260528-143412/v0-20260528-143800/checkpoint-810}"
base_model="${BASE_MODEL:-/mnt/jfs/copilot/lhb/checkpoint/opensource/Qwen3-VL-4B-Instruct}"
...
run_name="${RUN_NAME:-gui-sd-qwen3-4b-avvg_refgeo_sft_mrpd_metric_gaussian_e1_resume810_aos-${timestamp}}"
rjob_name="${RJOB_NAME:-q4b-avvg-mrpd-resume810-aos-${timestamp}}"
...
echo "[avvg-mrpd-aos-resume810] timestamp=${timestamp}"
echo "[avvg-mrpd-aos-resume810] source_ckpt=${source_ckpt}"
echo "[avvg-mrpd-aos-resume810] base_model=${base_model}"
echo "[avvg-mrpd-aos-resume810] run_name=${run_name}"

RUN_TIMESTAMP="$timestamp" \
EXPERIMENT_SLUG="avvg-refgeo-mrpd-resume810-aos" \
RUN_NAME="$run_name" \
RJOB_NAME="$rjob_name" \

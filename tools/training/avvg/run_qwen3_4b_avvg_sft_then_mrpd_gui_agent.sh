#!/usr/bin/env bash
# AVVG-refGeo experiment pipeline:
#   1. SFT Qwen3-VL-4B from the original base checkpoint on avvg_train.jsonl.
#   2. Continue from the SFT checkpoint with the current best stable MRPD setup
#      (GRPO + OPSD, metric teacher refresh, gaussian hint) on avvg_train.jsonl.
#   3. Launch a detached greedy evaluation on avvg_test.jsonl for the MRPD ckpt.
#
# The pipeline is intended to run inside a tmux session on the submit host. The
# actual training/eval jobs are submitted as gui_agent rjobs, so GPU resources are
...
val_jsonl="${VAL_JSONL:-/data/codes/gui_grounding/data/avvg_refgeo_full/avvg_train.jsonl}"
test_jsonl="${TEST_JSONL:-/data/codes/gui_grounding/data/avvg_refgeo_full/avvg_test.jsonl}"

sft_run_name="${SFT_RUN_NAME:-gui-sd-qwen3-4b-avvg_refgeo_sft_e1-${timestamp}}"
mrpd_run_name="${MRPD_RUN_NAME:-gui-sd-qwen3-4b-avvg_refgeo_sft_mrpd_metric_gaussian_e1-${timestamp}}"
sft_rjob_name="${SFT_RJOB_NAME:-q4b-avvg-sft-2n-${timestamp}}"
mrpd_rjob_name="${MRPD_RJOB_NAME:-q4b-avvg-sftmrpd-2n-${timestamp}}"

sft_ckpt_root="${SFT_CKPT_ROOT:-/mnt/jfs/copilot/lhb/checkpoint/rs/avvg-sft}"
mrpd_ckpt_root="${MRPD_CKPT_ROOT:-/mnt/jfs/copilot/lhb/checkpoint/rs/avvg-mrpd}"
sft_artifact_root="${SFT_ARTIFACT_ROOT:-/mnt/jfs/copilot/lhb/artifacts/rs/avvg-sft}"
mrpd_artifact_root="${MRPD_ARTIFACT_ROOT:-/mnt/jfs/copilot/lhb/artifacts/rs/avvg-mrpd}"
...
pipeline_log="${PIPELINE_LOG_PATH:-${train_log_root}/q4b-avvg-sft-then-mrpd-${timestamp}.pipeline.log}"
...
{
    echo "[avvg-pipeline] timestamp=${timestamp}"
    echo "[avvg-pipeline] base_model=${base_model}"
    echo "[avvg-pipeline] train_jsonl=${train_jsonl}"
    echo "[avvg-pipeline] val_jsonl=${val_jsonl}"
    echo "[avvg-pipeline] test_jsonl=${test_jsonl}"
    echo "[avvg-pipeline] sft_run_name=${sft_run_name}"
    echo "[avvg-pipeline] mrpd_run_name=${mrpd_run_name}"
    echo "[avvg-pipeline] sft_log=${sft_log}"
    echo "[avvg-pipeline] mrpd_log=${mrpd_log}"

    if [ ! -f "$train_jsonl" ] || [ ! -f "$test_jsonl" ]; then
        echo "ERROR: AVVG jsonl files are missing." >&2
        exit 1
    fi

    echo "[avvg-pipeline] stage1=SFT submit"
    RUN_TIMESTAMP="$timestamp" \
    EXPERIMENT_SLUG="avvg-refgeo-sft" \
    ...
    RJOB_CPU="${SFT_RJOB_CPU:-64}" \
    RJOB_GPU="${SFT_RJOB_GPU:-8}" \
    RJOB_MEMORY="${SFT_RJOB_MEMORY:-800000}" \
    ...
    MASTER_PORT="${SFT_MASTER_PORT:-30610}" \
    ...
    echo "[avvg-pipeline] sft_ckpt=${sft_ckpt}"

    echo "[avvg-pipeline] stage2=MRPD submit"
    RUN_TIMESTAMP="$timestamp" \
    EXPERIMENT_SLUG="avvg-refgeo-sft-mrpd-metric-gaussian" \
    ...
    RJOB_GROUP="${RJOB_GROUP:-gui_agent}" \
    RJOB_CHARGED_GROUP="${RJOB_CHARGED_GROUP:-gui_agent}" \
    RJOB_CPU="${MRPD_RJOB_CPU:-64}" \
    RJOB_GPU="${MRPD_RJOB_GPU:-8}" \
    RJOB_MEMORY="${MRPD_RJOB_MEMORY:-800000}" \
    ...
    MASTER_PORT="${MRPD_MASTER_PORT:-30620}" \
    VLLM_SERVER_PORT="${MRPD_VLLM_SERVER_PORT:-9062}" \
    ...
    echo "[avvg-pipeline] mrpd_ckpt=${mrpd_ckpt}"

    eval_slug="q4b-avvg-sft-mrpd-${timestamp}-test"
    eval_out="${eval_log_root}/${eval_slug}"
    eval_rjob="${EVAL_RJOB_NAME:-eval-avvg-mrpd-$(date +%m%d%H%M)}"
    echo "[avvg-pipeline] stage3=test eval submit eval_rjob=${eval_rjob} eval_out=${eval_out}"
    MODEL="$mrpd_ckpt" \
    RJOB_NAME="$eval_rjob" \
    ...
    cat <<EOF
[avvg-pipeline] completed
[avvg-pipeline] sft_ckpt=${sft_ckpt}
[avvg-pipeline] mrpd_ckpt=${mrpd_ckpt}
[avvg-pipeline] test_summary=${summary}
EOF

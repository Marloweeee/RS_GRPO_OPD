#!/usr/bin/env bash
# zoom_in staged restart using two independent rjobs.
#
# Stage 1: base Qwen3-VL-4B -> 80 steps with zoom_in hints.
...
base_run_name="${RUN_NAME:-gui-sd-qwen3-4b-rsfull_grpo_sdpo_zoom_2stage_seq_s80_e1-${timestamp}}"
stage1_run_name="${STAGE1_RUN_NAME:-${base_run_name}_stage1_zoom_s80}"
stage2_run_name="${STAGE2_RUN_NAME:-${base_run_name}_stage2_zoom_from_s80_e1}"
base_rjob_name="${RJOB_NAME:-q4b-z2s80-${timestamp}}"
...
run_stage() {
    ...
    local rjob_name="$3"
    local model_path="$4"
    local teacher_path="$5"
    shift 5

    log "launching ${stage}: run_name=${run_name} rjob_name=${rjob_name}"
    RUN_TIMESTAMP="$timestamp" \
    EXPERIMENT_SLUG="zoom-${stage}" \
    RUN_NAME="$run_name" \
    RJOB_NAME="$rjob_name" \
...
run_stage \
    ...
    env \
        ...
        VLLM_SERVER_PORT="${STAGE1_VLLM_SERVER_PORT:-8541}" \
        MASTER_PORT="${STAGE1_MASTER_PORT:-29541}"
...
run_stage \
    ...
    env \
        ...
        SAVE_ONLY_MODEL=true \
        VLLM_SERVER_PORT="${STAGE2_VLLM_SERVER_PORT:-8542}" \
        MASTER_PORT="${STAGE2_MASTER_PORT:-29542}"

log "zoom staged sequential rjobs run completed"

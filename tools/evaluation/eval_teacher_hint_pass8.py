...
def render_hint_image(image, bbox_pixel, mode, sigma_ratio=1.5, min_area_frac=0.1,
                      zoom_ratio=2.0, jitter_ratio=0.2, hint_box_color="magenta"):
...
def threshold_metrics(ious, prefix=""):
    ...
    for threshold in (0.5, 0.6, 0.7, 0.8, 0.9):
        metrics[f"{prefix}IoU@{threshold:.1f}"] = round(
            sum(i > threshold for i in ious) / max(1, len(ious)), 4)
    return metrics
...
def main():
    ...
    parser.add_argument("--hint_mode", default="gaussian",
    ...
    def build_request(sample):
        ...
        messages = [
            ...
            {"role": "user", "content": [
                {"type": "image"},
                {"type": "text", "text": build_hint_text(sample["user_text"], args.hint_mode, args.opsd_hint_box_color)},
            ]},
        ]

...
DEFAULT_METHODS = {
    "sft": EVAL_ROOT / "qwen3-4b-rrsisd-sft-20260526-220915-checkpoint-96-test-greedy" / "per_sample.jsonl",
    "opsd_only": EVAL_ROOT / "q4b-sft-opsd-only-gaussian-20260527-085701-test" / "per_sample.jsonl",
    "grpo_only": EVAL_ROOT / "q4b-sft-grpo-only-20260527-082554-test" / "per_sample.jsonl",
    "mrpd_metric": EVAL_ROOT / "gui-sd-qwen3-4b-rrsisd_sft_grpo_opsd_metric_gaussian_e1-20260526-225857_test" / "per_sample.jsonl",
    "mrpd_ema098": EVAL_ROOT / "gui-sd-qwen3-4b-rrsisd_sft_grpo_opsd_ema098_gaussian_e1-20260527-113500-ema098_test" / "per_sample.jsonl",
    "mrpd_no_refresh": EVAL_ROOT / "gui-sd-qwen3-4b-rrsisd_sft_grpo_opsd_no_refresh_gaussian_e1-20260527-114200-norefresh_test" / "per_sample.jsonl",
    "mrpd_step80": EVAL_ROOT / "gui-sd-qwen3-4b-rrsisd_sft_grpo_opsd_fixed_s80_gaussian_e1-20260527-113500-s80_test" / "per_sample.jsonl",
    "mrpd_zoom": EVAL_ROOT / "q4b-sft-grpo-opsd-zoom-in-20260527-082554-test" / "per_sample.jsonl",
    "mrpd_jitter": EVAL_ROOT / "q4b-sft-grpo-opsd-jitter-box-20260527-082554-test" / "per_sample.jsonl",
    "mrpd_soft_window": EVAL_ROOT / "q4b-sft-grpo-opsd-soft-window-20260527-100136-test" / "per_sample.jsonl",
    "mrpd_no_hint": EVAL_ROOT / "q4b-sft-grpo-opsd-no-hint-20260527-093400-aos2n-test" / "per_sample.jsonl",
}
...
def build_report_html(results: Dict[str, Any]) -> str:
    style = """
    body{margin:0;background:#f6f8fb;color:#182233;font:14px/1.55 -apple-system,BlinkMacSystemFont,"Segoe UI","Noto Sans SC",Arial,sans-serif}
    ...
    th,td{border-bottom:1px solid #d8e0ea;border-right:1px solid #edf1f5;padding:7px 8px;text-align:left;vertical-align:top}
    ...
    bootstrap_rows = [
        ...
        for name, value in results["paired_bootstrap"].items()
    ]
    split_overlap_rows = [
        {"pair": name, **value}
        for name, value in results["rsvg_split_leakage"]["overlap"].items()
    ]
    method_rows = [
        {"method": name, **summary}
        for name, summary in results["method_summaries"].items()
    ]
...
<section><h2>Paired Bootstrap 显著性</h2><p>mean_delta = other - base。p 值为 bootstrap 双侧近似，主要用于论文阶段判断差异是否稳定。</p>{html_table(bootstrap_rows)}</section>
<section><h2>RSVG-DIOR Split 泄漏检查</h2>{html_table(split_overlap_rows)}<pre>{html.escape(json.dumps(results["rsvg_split_leakage"]["splits"], ensure_ascii=False, indent=2))}</pre></section>
...
def main() -> None:
    ...
    method_summaries = {
        name: metric_summary(rows.values())
    ...
    analysis_methods = {
        key: methods[key]
        for key in ("sft", "grpo_only", "mrpd_metric", "mrpd_ema098")
        if key in methods
    }
    bucket_analysis = {
        "area": grouped_metric_table(analysis_methods, "sft", lambda row: area_bucket(gt_box(row)) if gt_box(row) else None),
        "aspect": grouped_metric_table(analysis_methods, "sft", lambda row: aspect_bucket(gt_box(row)) if gt_box(row) else None),
        "edge": grouped_metric_table(analysis_methods, "sft", lambda row: edge_bucket(gt_box(row)) if gt_box(row) else None),
        "category": grouped_metric_table(analysis_methods, "sft", category, min_count=20),
    }
    ...
    results = {
        ...
        "config": {
            ...
            "seed": args.seed,
            "method_files": {k: str(v) for k, v in DEFAULT_METHODS.items()},
        },
    }

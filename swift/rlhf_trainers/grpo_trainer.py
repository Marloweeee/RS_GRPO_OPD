...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
                 ...
                 teacher_deepspeed_config: Optional[dict] = None,
        ...
        self._prepare_sdpo_teacher(teacher_model, teacher_deepspeed_config)
    ...
    def _prepare_sdpo_teacher(self, teacher_model, teacher_deepspeed_config=None):
        ...
        if teacher_model is None:
            raise ValueError('`--use_sdpo true` requires `--teacher_model` for GRPO+SDPO training.')
        if self.is_deepspeed_enabled:
            if teacher_deepspeed_config is not None:
                ...
                self.teacher_model = prepare_teacher_deepspeed(
                    teacher_model, self.accelerator, deepspeed_config=teacher_deepspeed_config, training_args=self.args)
            else:
                self.teacher_model = prepare_teacher_deepspeed(teacher_model, self.accelerator)
    ...
    def _generate_and_score_completions(self, inputs: DataType) -> DataType:
        ...
        self._prepare_sdpo_routing(inputs, total_rewards_per_func)
        ...
        for batch, batch_encoded in zip(gas_chunks, batch_encoded_inputs):
            ...
            self._attach_sdpo_batch(batch, batch_encoded)
    ...
    def _prepare_sdpo_routing(self, inputs: DataType, total_rewards_per_func: torch.Tensor) -> None:
        if not getattr(self, 'use_sdpo', False):
            return
        if self.dynamic_num_samples:
            logger.warning('SDPO routing is disabled for dynamic_num_samples batches.')
            return
        ...
        local_rewards = get_even_process_data(self, rewards.detach().cpu().tolist())
        if len(local_rewards) != len(inputs):
            logger.warning(f'SDPO routing skipped: local reward count {len(local_rewards)} != inputs {len(inputs)}')
            return

        routes = []
        ious = []
        for data, reward in zip(inputs, local_rewards):
            iou, valid_box, pred_bbox = self._sdpo_score_completion(data)
            data['_sdpo_iou'] = iou
            data['_sdpo_valid_box'] = valid_box
            data['_sdpo_pred_bbox'] = pred_bbox
            data['_sdpo_reward'] = float(reward)
        ...
        for start in range(0, len(inputs), num_generations):
            ...
            for data in group:
                ...
                data['_sdpo_advantage'] = adv
                if data['_sdpo_valid_box'] and data['_sdpo_iou'] >= self.args.sdpo_tau_good \
                        and data['_sdpo_reward'] >= group_mean:
                    route = 'good'
                elif (not data['_sdpo_valid_box']) or data['_sdpo_iou'] < self.args.sdpo_tau_fail \
                        or adv < -self.args.sdpo_delta:
                    route = 'failed'
                else:
                    route = 'ambiguous'
    ...
    def _sdpo_score_completion(self, data):
        ...
        from swift.cus_rewards.bbox_iou_reward import _iou_and_giou
        ...
        if not isinstance(completion, str):
            try:
                if isinstance(completion, list):
                    ...
                    completion = self.processing_class.decode(completion['token_ids'])
            except Exception:
                completion = ''
        ...
        try:
            ...
            gt = bboxreal2norm(gt_pixel, additional['image_size'])
            iou = _iou_and_giou(pred_bbox, gt)[0]
        except Exception:
            return 0.0, valid_box, pred_bbox
        return iou, valid_box, pred_bbox

    def _attach_sdpo_batch(self, batch: DataType, batch_encoded: Dict[str, torch.Tensor]) -> None:
        if not getattr(self, 'use_sdpo', False):
            return
    ...
    def _compute_sdpo_loss(self, model, inputs):
        seq_mask = inputs.get('sdpo_sequence_mask')
        origin_batch = inputs.get('_sdpo_origin_batch')
        if seq_mask is None or origin_batch is None:
            return torch.zeros((), dtype=torch.float32, device=self.accelerator.device)

        global_seq_mask_sum = seq_mask.detach().sum()
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            torch.distributed.all_reduce(global_seq_mask_sum, op=torch.distributed.ReduceOp.SUM)
        if global_seq_mask_sum.item() <= 0:
            return torch.zeros((), dtype=torch.float32, device=self.accelerator.device)

        if self.args.sdpo_target == 'gt':
            student_inputs, teacher_inputs = self._prepare_sdpo_gt_inputs(origin_batch)
        ...
        teacher_model_inputs = self._prepare_sdpo_model_inputs(teacher_inputs)
        ...
        valid_len = min(student_logits.shape[1], teacher_logits.shape[1])
        student_logits = student_logits[:, -valid_len:, :]
        teacher_logits = teacher_logits[:, -valid_len:, :]
        student_labels = student_inputs['labels'][:, -valid_len:]
        teacher_labels = teacher_inputs['labels'][:, -valid_len:]
        token_mask = (student_labels != -100) & (teacher_labels != -100)
        token_weights = self._sdpo_token_weights(student_labels, teacher_logits, token_mask)
        sequence_weights = seq_mask.to(token_weights.device).unsqueeze(1)
        token_weights = token_weights * sequence_weights * token_mask.to(token_weights.dtype)

        if token_weights.sum().item() <= 0:
            return torch.zeros((), dtype=torch.float32, device=student_logits.device)

        active_mask = token_weights > 0
        student_logits = student_logits[active_mask]
        teacher_logits = teacher_logits[active_mask]
        flat_weights = token_weights[active_mask]

        return self._sdpo_topk_teacher_kl_loss(
            student_logits=student_logits,
            teacher_logits=teacher_logits,
            weights=flat_weights,
            temperature=self.temperature,
            top_k=getattr(self.args, 'sdpo_top_k', 64),
        )

    @staticmethod
    def _sdpo_topk_teacher_kl_loss(
        ...
        """Distill teacher preference on failed rollouts with a bounded top-k KL.

        Full-vocabulary JSD is expensive and can be numerically brittle for bf16
        VLM logits. A top-k teacher KL keeps the correction distributional: the
        student is pushed toward the teacher's high-probability bbox tokens rather
        than toward the failed sampled token itself.
        """
        if student_logits.numel() == 0 or weights.numel() == 0:
            return torch.zeros((), dtype=torch.float32, device=student_logits.device)
        ...
        student_logits = torch.nan_to_num(
            student_logits.float(), nan=0.0, posinf=80.0, neginf=-80.0).clamp_(-80.0, 80.0) / temperature
        teacher_logits = torch.nan_to_num(
            teacher_logits.float(), nan=0.0, posinf=80.0, neginf=-80.0).clamp_(-80.0, 80.0) / temperature
        weights = torch.nan_to_num(weights.float(), nan=0.0, posinf=0.0, neginf=0.0).clamp_(0.0, 10.0)

        teacher_top_values, teacher_top_indices = torch.topk(teacher_logits, k=top_k, dim=-1)
        teacher_log_probs = F.log_softmax(teacher_top_values, dim=-1)
        teacher_probs = teacher_log_probs.exp()

        student_log_probs = F.log_softmax(student_logits, dim=-1).gather(-1, teacher_top_indices)
        token_kl = (teacher_probs * (teacher_log_probs - student_log_probs)).sum(dim=-1)
        token_kl = torch.nan_to_num(token_kl, nan=0.0, posinf=0.0, neginf=0.0).clamp_min_(0.0)

        weighted_loss = token_kl * weights
        return weighted_loss.sum() / weights.sum().clamp(min=1.0)

    def _prepare_sdpo_model_inputs(self, encoded):
        excluded = {
            'labels',
            'completion_mask',
            'truncated_mask',
        ...
        model_inputs = {k: v for k, v in encoded.items() if k not in excluded}
        if 'logits_to_keep' in self.model_kwarg_keys and 'labels' in encoded:
            labels = encoded['labels']
            logits_to_keep = (labels.shape[-1] - (torch.ne(labels, -100).int().argmax(-1))).max().item()
            model_inputs['logits_to_keep'] = logits_to_keep + 1
        return model_inputs

    def _prepare_sdpo_student_inputs(self, origin_batch: DataType):
        encoded_inputs = []
        with self._template_context(self.template):
            for data in origin_batch:
                student_data = deepcopy(data)
                self._replace_sdpo_response_with_ids(student_data)
                encoded_inputs.append(self.template.encode(student_data, return_length=True))
            return to_device(self.template.data_collator(encoded_inputs), self.model.device)

    def _prepare_sdpo_teacher_inputs(self, origin_batch: DataType):
        encoded_inputs = []
        with self._template_context(self.template):
            for data in origin_batch:
                teacher_data = self._make_sdpo_teacher_data(data)
                self._replace_sdpo_response_with_ids(teacher_data)
                encoded_inputs.append(self.template.encode(teacher_data, return_length=True))
            return to_device(self.template.data_collator(encoded_inputs), self.model.device)
    ...
    def _prepare_sdpo_gt_inputs(self, origin_batch: DataType):
        ...
        with self._template_context(self.template):
            for data in origin_batch:
                if data.get('_sdpo_route') != 'failed':
                    continue
                gt_data = self._make_sdpo_gt_data(data)
                teacher_data = self._make_sdpo_teacher_data(gt_data)
                student_encoded.append(self.template.encode(gt_data, return_length=True))
                teacher_encoded.append(self.template.encode(teacher_data, return_length=True))
        if not student_encoded:
            empty = {'labels': torch.empty((0, 0), dtype=torch.long, device=self.accelerator.device)}
            return empty, empty
        return (
            to_device(self.template.data_collator(student_encoded), self.model.device),
            to_device(self.template.data_collator(teacher_encoded), self.model.device),
        )
    ...
    def _make_sdpo_teacher_data(self, data):
        ...
        if hint_mode == 'hint':
            ...
            if isinstance(teacher_data['messages'][1]['content'], str):
                teacher_data['messages'][1]['content'] += hint_text
            mask_img_path = self.sdpo_mask_processor(teacher_data)
            self._set_sdpo_image_path(teacher_data, mask_img_path)
            return teacher_data
        if hint_mode == 'gt':
            gt_text = self._sdpo_gt_response_text(teacher_data)
            if isinstance(teacher_data['messages'][1]['content'], str):
                teacher_data['messages'][1]['content'] += f'\n\nThe correct bbox answer: {gt_text}'
            mask_img_path = self.sdpo_mask_processor(teacher_data)
            self._set_sdpo_image_path(teacher_data, mask_img_path)
            return teacher_data
        raise ValueError(f"Unknown opsd_hint_mode: {hint_mode}, expected 'none', 'hint', or 'gt'")

    def _set_sdpo_image_path(self, data, image_path):
        if isinstance(data.get('images'), str):
            data['images'] = image_path
        elif isinstance(data.get('images'), dict):
            data['images']['path'] = image_path
        elif isinstance(data.get('images'), list) and data['images']:
            if isinstance(data['images'][0], dict):
                data['images'][0]['path'] = image_path
            else:
                data['images'][0] = image_path
        else:
            data['images'] = image_path

    def _make_sdpo_gt_data(self, data):
        gt_data = deepcopy(data)
        gt_text = self._sdpo_gt_response_text(gt_data)
        if gt_data.get('messages') and gt_data['messages'][-1].get('role') == 'assistant':
            gt_data['messages'][-1]['content'] = gt_text
        else:
            gt_data.setdefault('messages', []).append({'role': 'assistant', 'content': gt_text})
        gt_data.pop('response_token_ids', None)
        gt_data.pop('response_loss_mask', None)
        return gt_data

    def _sdpo_gt_response_text(self, data):
        from swift.custom_utils.ground_func import bboxreal2norm
        additional = data.get('additional_paras', '{}')
        if isinstance(additional, str):
            additional = json.loads(additional)
        bbox = bboxreal2norm(data['solution']['arguments']['coordinate'], additional['image_size'])
        bbox = [int(v) for v in bbox]
        return json.dumps({'bbox_2d': bbox}, separators=(',', ': '))

    def _sdpo_token_weights(self, shifted_labels, teacher_logits, teacher_mask):
        ...
        max_digit_len = getattr(self.args, 'opsd_max_digit_len', 0)

        if 'linear' in weight_mode:
            weights = torch.full_like(shifted_labels, non_digit_w, dtype=torch.float32)
            seqs = shifted_labels.tolist()
                ...
                while i < len(seq):
                    if seq[i] in self._digit_tokens:
                        ...
                        while j < len(seq) and seq[j] in self._digit_tokens:
                            j += 1
                        n_digits = j - i
                        effective_n = min(n_digits, max_digit_len) if max_digit_len > 0 else n_digits
                        for k in range(n_digits):
                            weights[b_idx, i + k] = float(min(n_digits - k, effective_n))
                        i = j
                    else:
                        i += 1
        else:
            weights = torch.ones_like(shifted_labels, dtype=torch.float32)
        ...
        if 'entropy' in weight_mode:
            ...
            token_entropy = -(teacher_probs * torch.log(teacher_probs + 1e-8)).sum(dim=-1)
            ...
            confidence = 1.0 - token_entropy / h_max
            valid_weights = weights[valid_mask].to(confidence.device)
            valid_len = min(valid_weights.numel(), confidence.numel())
            weighted_values = valid_weights.new_zeros(valid_weights.shape)
            if valid_len > 0:
                weighted_values[:valid_len] = valid_weights[:valid_len] * confidence[:valid_len]
            weights = torch.zeros_like(weights, dtype=torch.float32, device=confidence.device)
            weights[valid_mask] = weighted_values

        return weights.to(teacher_logits.device)
    ...
    def _update_metrics(self, metrics_data):
        ...
        if 'sdpo_loss' in metrics_data:
            self._metrics[mode]['sdpo/loss'].append(metrics_data['sdpo_loss'])
    ...
    def sdpo_mask_processor(self, data):
        ...
        if mask_mode == 'zoom_in':
            ...
            if crop_x1 < crop_x2 and crop_y1 < crop_y2:
                res.paste(img.crop((crop_x1, crop_y1, crop_x2, crop_y2)), (crop_x1, crop_y1))
            ...
            pad_w = max(int(bbox_w * zoom_ratio), int(W * math.sqrt(min_area_frac) / 2))
            pad_h = max(int(bbox_h * zoom_ratio), int(H * math.sqrt(min_area_frac) / 2))
            ...
            if crop_x1 < crop_x2 and crop_y1 < crop_y2:
                res.paste(img.crop((crop_x1, crop_y1, crop_x2, crop_y2)), (crop_x1, crop_y1))
        elif mask_mode in {'gaussian', 'soft_window'}:
            sigma_ratio = getattr(self.args, 'opsd_gaussian_sigma_ratio', 1.5)
            min_area_frac = getattr(self.args, 'opsd_min_area_frac', 0.1)
            sigma = max(max(bbox_w, bbox_h) * sigma_ratio, min(W, H) * math.sqrt(min_area_frac))
            ...
            res = img.copy()
            jitter_ratio = getattr(self.args, 'opsd_jitter_ratio', 0.2)
            jw_max = max(1, int(bbox_w * jitter_ratio))
            jh_max = max(1, int(bbox_h * jitter_ratio))
            jx1 = max(0, min(W - 1, x1 + random.randint(-jw_max, jw_max)))
            jy1 = max(0, min(H - 1, y1 + random.randint(-jh_max, jh_max)))
            jx2 = max(jx1 + 1, min(W, x2 + random.randint(-jw_max, jw_max)))
            jy2 = max(jy1 + 1, min(H, y2 + random.randint(-jh_max, jh_max)))
            color = getattr(self.args, 'opsd_hint_box_color', 'magenta')
            draw_box = (jx1, jy1, jx2, jy2, color)
        else:
            res = img.copy()
    ...
    def _get_logps_via_local_forward(self,
        ...
        if getattr(self, 'use_sdpo', False) and model.training and getattr(self.args, 'sdpo_lambda', 0) > 0:
            inputs['_sdpo_student_logits'] = logits
        logits = logits / self.temperature
    ...
    def generalized_jsd_loss(
        ...
        for start_idx in range(0, num_valid_int, chunk_size):
            ...
            jsd_chunk = torch.nan_to_num(jsd_chunk, nan=0.0, posinf=0.0, neginf=0.0).sum(dim=-1)
            if weights is not None:
                jsd_chunk = jsd_chunk * weights[start_idx:end_idx].float()
            total_loss = total_loss + jsd_chunk.sum()

        if weights is not None:
            normalizer = weights[:num_valid_int].float().sum().clamp(min=1.0)

...
class OPSDTrainer(RolloutTrainerMixin, SwiftMixin, HFGKDTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_liger_gkd_loss:
            ...
            # === 监控信号：teacher 数字 token 熵 / GT 置信度 / 自身 IoU ===
            # 用于诊断 hint 是否过强（熵跌穿 → 接近 hard label，蒸馏失效）
            # 不按 data_source 过滤：教师恒看 hint 版本，监控对所有路径都有效
            if getattr(self.args, 'opsd_monitor_teacher', False) \
                    and getattr(self, '_monitor_gt_bboxes', None):
                try:
                    self._compute_teacher_monitor(
                        outputs_teacher=outputs_teacher,
                        shifted_teacher_labels=shifted_teacher_labels,
    ...
    def _compute_teacher_monitor(self, outputs_teacher, shifted_teacher_labels, mask_teacher):
        """记录三个监控指标到 self._metrics：
           - teacher/digit_entropy : 数字 token 上 teacher 分布熵均值（越高 = 越软）
           - teacher/digit_p_at_gt : 数字 token 上 teacher 给到 GT label 的概率均值（越高 = 越像 hard label）
           - teacher/iou           : teacher 自身贪心解码 bbox 与 GT 的 IoU 均值（≥0.95 报警）
        """
        ...
        with torch.no_grad():
            # 数字 token 位置 mask
            digit_pos_mask = torch.zeros_like(shifted_teacher_labels, dtype=torch.bool)
            teacher_labels_cpu = shifted_teacher_labels.tolist()
            for b_idx, seq in enumerate(teacher_labels_cpu):
                for i_idx, t in enumerate(seq):
                    if t in self._digit_tokens:
                        digit_pos_mask[b_idx, i_idx] = True
            ...
            if digit_mask.any():
                ...
                t_p_at_gt = t_probs.gather(-1, gt_labels.unsqueeze(-1)).squeeze(-1)
                self._metrics[mode]['teacher/digit_entropy'].append(t_entropy.mean().item())
                self._metrics[mode]['teacher/digit_p_at_gt'].append(t_p_at_gt.mean().item())
                del t_logits, t_probs, t_entropy, t_p_at_gt

            # 贪心解码 teacher 的 response 段 → 解析 bbox → 与 GT 比 IoU
            ious = []
            B = shifted_teacher_labels.shape[0]
            gt_list = self._monitor_gt_bboxes
            for b_idx in range(B):
                m = mask_teacher[b_idx]
                if not m.any() or b_idx >= len(gt_list) or gt_list[b_idx] is None:
                    continue
                pred_ids = outputs_teacher.logits[b_idx][m].argmax(dim=-1).tolist()
                try:
                    text = self.template.tokenizer.decode(pred_ids)
                except Exception:
                    continue
                pred_bbox = extract_bbox(text)
                if pred_bbox == 'no bbox':
                    continue
                iou, _ = _iou_and_giou(pred_bbox, gt_list[b_idx])
                ious.append(iou)
            if ious:
                self._metrics[mode]['teacher/iou'].append(sum(ious) / len(ious))
    ...
    def mask_processor(self, data):
        ...
        # 默认在 GT 框位置画绿框；soft_window / no_mask 不画；jitter_box 用抖动框+品红
        ...
        if mask_mode == 'zoom_in':
            ...
            # 方案 A：高斯衰减（同 gaussian）但**不画框**，避免泄漏 GT 精确位置
            # 教师"知道目标大概在亮的区域"，但 bbox 形状/边界仍要自己估
            sigma_ratio = getattr(self.args, 'opsd_gaussian_sigma_ratio', 1.5)
            min_area_frac = getattr(self.args, 'opsd_min_area_frac', 0.1)
            sigma = max(bbox_w, bbox_h) * sigma_ratio
            min_sigma = min(W, H) * math.sqrt(min_area_frac)
            ...
            # 方案 C：原图 + 在 GT 周围画一个**抖动后的框**作为模糊提示
            # 教师需把抖动框"收紧"回真实 bbox，KD 信号保留在尺寸/位置 logits 上
            res = img.copy()
    ...
    def _prepare_batch_inputs(self, inputs: list, encode_prompt_only: bool = False) -> Dict[str, torch.Tensor]:
        ...
        # encode_prompt_only=False → 编码完整 messages（已经有 response 的情况，直接拿来算 loss）
        from .utils import replace_assistant_response_with_ids
        # 把 messages 里 assistant 那段 text 替换成已 tokenize 好的 token_ids
        # （vLLM rollout 完是 token_ids，不是字符串，直接塞回去避免重新 tokenize 引入误差）
        ...
        # 'train' 模式：为前向计算服务，response 段保留为真 label
        with self._template_context(template, mode=mode):
            # 上下文管理器：进出时切换 template 的 mode、max_length 等内部状态
            for data in inputs:
                # ------ 处理 Student 的输入（原图 + 原 prompt） ------
                student_data = deepcopy(data)  # 深拷贝，避免对原 dict 的修改影响 teacher 那一路
                ...
                # 调 template 走完整流程：chat template → tokenize → 处理 image → 生成 input_ids/labels/pixel_values
                # return_length=True 多返回长度信息，便于后续 padding
                student_encoded_inputs.append(student_encoded)

                # ------ 处理 Teacher 的输入（可能加 hint / 遮罩图） ------
                teacher_data = deepcopy(data)  # 再深拷贝一份原始 data，准备做"teacher 专属"的改造
                hint_mode = getattr(self.args, 'opsd_hint_mode', 'hint')
                # OPSD 关键超参：teacher 看到的版本要不要带提示
                #   'none' → 和 student 一样（退化为标准 GKD）
                ...
                if 'messages' in teacher_data:
                    ...
                    if hint_mode == 'none':
                        # 不加任何hint，teacher输入与student完全一致（退化为标准 GKD）
                        pass
                    elif hint_mode == 'hint':
                        ...
                        #   1) 按 opsd_mask_mode 做 zoom_in / adaptive / gaussian / soft_window / jitter_box / no_mask 处理
                        #   2) 根据 mode 决定是否画框，以及框的位置/颜色
                        # 返回处理后的图片路径
                        teacher_data['images'][0]['path'] = mask_img_path
                        # 把图像路径替换成处理后的图，teacher 看的就是这张简化版
                    elif hint_mode == 'gt':
                        # 加绿框遮罩图 + 直接给出gt归一化中心坐标（比 hint 更激进，几乎照抄答案）
                        gt_bbox = teacher_data['solution']['arguments']['coordinate']
                        # 取 GT 的 [x1,y1,x2,y2]（原图坐标系，绝对像素）
                        additional = teacher_data.get('additional_paras', '{}')
                        if isinstance(additional, str):
                            additional = json.loads(additional)
                        image_size = additional['image_size']
                        # 取原图宽高，后面要把绝对坐标归一化到 norm1000
                        # 计算归一化中心点 (与数据构建一致: bbox→norm1000→中心)
                        norm_bbox = [
                            int(gt_bbox[0] / image_size[0] * 1000),  # x1_norm
                            int(gt_bbox[1] / image_size[1] * 1000),  # y1_norm
                            int(gt_bbox[2] / image_size[0] * 1000),  # x2_norm
                            int(gt_bbox[3] / image_size[1] * 1000),  # y2_norm
                        ]
                        # 转到 0~1000 的归一化坐标系（必须和模型训练用的坐标格式一致）
                ...
                # 走完整 encode 流程（注意：此时图像已是 mask 图，prompt 已带 hint）
                teacher_encoded_inputs.append(teacher_encoded)

            # 分别进行 padding 和 collate
            # 注意：student 和 teacher 的 prompt 长度不同（teacher 多了 hint 文本和不同的图）
            # 所以是两个独立的 batch，各自 padding，各自有自己的 attention_mask 和 labels
            # 后续 compute_loss 靠 labels != -100 的 mask 去抽 response 段对齐 GJSD
            # 收集每条样本的 GT norm-1000 bbox 供 compute_loss 里的监控使用
            # 顺序与 student_encoded_inputs / teacher_encoded_inputs 一致
            if getattr(self.args, 'opsd_monitor_teacher', False):
                gt_norms = []
                for data in inputs:
                    try:
                        ap = data.get('additional_paras', '{}')
                        if isinstance(ap, str):
                            ap = json.loads(ap)
                        W_, H_ = ap['image_size']
                        bx = data['solution']['arguments']['coordinate']
                        gt_norms.append([
                            int(bx[0] / W_ * 1000),
                            int(bx[1] / H_ * 1000),
                            int(bx[2] / W_ * 1000),
                        ...
                        gt_norms.append(None)
                self._monitor_gt_bboxes = gt_norms
            else:
                self._monitor_gt_bboxes = []

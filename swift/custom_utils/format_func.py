...
_BOX_TOOLCALL_RE = re.compile(
    r"<tool_call>\s*\(\s*(-?\d+)\s*,\s*(-?\d+)\s*\)\s*,\s*\(\s*(-?\d+)\s*,\s*(-?\d+)\s*\)"
)
...
def extract_bbox(text):
    ...
    if not isinstance(text, str):
        return "no bbox"
    m = (_BOX_JSON_RE.search(text)
         or _BOX_QWEN_RE.search(text)
         or _BOX_XML_RE.search(text)
         or _BOX_TOOLCALL_RE.search(text)
         or _BOX_PAIR_RE.search(text))
    if not m:
        return "no bbox"
    try:

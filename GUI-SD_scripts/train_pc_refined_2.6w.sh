#!/bin/bash
run_name="gui-sd-8b_pc_refined_2.6w"

# 断点恢复：checkpoint 为 save_only_model，无优化器分片，必须加 resume_only_model。
# 新跑全量请执行：RESUME_FROM_CHECKPOINT= bash "$0" 或把下面默认值改为空。
RESUME_FROM_CHECKPOINT="${RESUME_FROM_CHECKPOINT:-/data/codes/gui_grounding/output/gui-sd-8b_pc_refined_2.6w/v0-20260513-003238/checkpoint-180}"
extra_train_args=()
if [ -n "${RESUME_FROM_CHECKPOINT}" ]; then
    extra_train_args+=(--resume_from_checkpoint "${RESUME_FROM_CHECKPOINT}")
    extra_train_args+=(--resume_only_model true)
fi
...
swift rlhf \
    ...
    --output_dir /mnt/jfs/copilot/yhl/checkpoint/copilot/guisd/${run_name} \
    --warmup_ratio 0.05 \
